...
export default function Navigation() {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${scrolled ? "bg-void/80 backdrop-blur-md border-b border-bone/[0.03]" : ""
        }`}
    >
        ...
        <div className="md:hidden fixed inset-0 bg-void/98 backdrop-blur-lg flex flex-col items-center justify-center gap-10 z-40">
          {links.map((link, i) => (
            <Link
              ...
              className={`group font-heading text-4xl tracking-[0.2em] transition-all duration-500 ${pathname === link.href ? "text-crimson" : "text-bone/30 hover:text-bone"
                }`}
